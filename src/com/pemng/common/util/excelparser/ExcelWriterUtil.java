...
public class ExcelWriterUtil {
	...
	private HSSFCellStyle c;
	
	/**
	 * @param log
	 */
	public ExcelWriterUtil(Log log) {
		this(log, "sheet1", false);
	}

	public ExcelWriterUtil(Log log, String sheetName) {
		this(log, sheetName, false);
	}
	
	public ExcelWriterUtil(Log log, String sheetName, boolean noSheet){
		this.log = log;
		wb = new HSSFWorkbook();
		if(!noSheet){
	 ...
	 * @coustructor 方法.
	 */
	public ExcelWriterUtil(InputStream templateInputStream, int sheetIdx,Integer n,boolean b) throws IOException {
		POIFSFileSystem fs = new POIFSFileSystem(templateInputStream);
		wb = new HSSFWorkbook(fs);
		HSSFSheet sheet = (HSSFSheet) wb.getSheetAt(sheetIdx);
		String sheetName = wb.getSheetName(sheetIdx);
		...
		c=(HSSFCellStyle) wb.createCellStyle();
		c.setFont(font);
		c.setAlignment(CellStyle.ALIGN_CENTER);
		curSheetWriter = new SheetWriter(sheetName, sheet);
	}
	 ...
	 * @param log
	 */
	public ExcelWriterUtil(InputStream templateInputStream, Log log)
			throws IOException {
	...
	public void setCurrentSheetTotalRow(int totalRow){
		curSheetWriter.setTotalRow(totalRow);
	}
	...
	public HSSFCellStyle getC() {
		return c;
	}

	public void setC(HSSFCellStyle c) {
		this.c = c;
	}
}
