package com.pemng.serviceSystem.base.struts2result;
 ...
 */
public class ChinacpJasperReportsResult extends StrutsResultSupport implements JasperReportConstants {

    private static final long serialVersionUID = -2523174799621182907L;

    private final static Logger LOG = LoggerFactory.getLogger(ChinacpJasperReportsResult.class);
    ...
    private String callbackName;  //可设置回调类进行处理、目前未实现
     ...
     * Default ctor.
     */
    public ChinacpJasperReportsResult() {
        super();
    }
     ...
     */
    public ChinacpJasperReportsResult(String location) {
        super(location);
    }
    ...
    //但valuestack包含需要写本地文件的路径时，先进行写入
    private void writeReport2File(byte[] output, ValueStack stack) throws IOException{
    	String filePath = (String)ServletActionContext.getRequest().getAttribute("chinacp.jasperreports.file.path");
    	if(filePath != null){
    		if(LOG.isInfoEnabled()){
    			LOG.info("JasperReport dealwith, and write byte to path [" + filePath + "]");
    		}
    		FileUtils.writeByteArrayToFile(new File(filePath), output);
    	}
    }
