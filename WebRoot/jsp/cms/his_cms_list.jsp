<!-- 
	历史委托书查询
	/PEMng/WebRoot/jsp/cms/his_cms_list.jsp
 -->
...
<html>
	...
	<script type="text/javascript">
		...
		function searchCms(){
			document.searchForm.action="${pageContext.request.contextPath}/cms/hisCmsList.action";
	...
	<h3>历史委托书查询</h3>
		...
		<div class="box3-tc" id="searchCloseDiv">
				<form action="${pageContext.request.contextPath}/cms/hisCmsList.action" method="post" name="searchForm" id="searchForm">
					<s:hidden name="cmsDto.cmsType" id="cmsDto.cmsType" value="0"/>
					<s:hidden name="cmsDto.flag" id="cmsDto.flag" value="4"/>
						<table>
							<tr>
								<th>委托内容：</th>
								<td>
							...
							<tr>
								<th>鉴定基准日:</th>
								<td colspan="3">
									<s:textfield name="cmsDto.beginTime" id="beginTime" readonly="true" cssClass="input1" size="12" /> 
				...
				<form  method="post" name="deleteCmsForm" id="deleteCmsForm">
					<div class="box1">
						<table id="cms_info" border="0" cellpadding="0" cellspacing="0">
							<tr>
								<th width="15%" align="center">鉴定文号</th>
								<th width="21%" align="center">委托内容</th>
								<th width="12%" align="center">委托单位简称</th>
							...
							<s:iterator value="cmList" status="status">
								<tr <s:if test="prtDt != null">class="tablerowhighlight"</s:if><s:elseif test="%{#status.index%2==0}">class="td1"</s:elseif>>
									<td style="text-align: center;" nowrap>
										...
										<c:if test="${sessionScope.SESSION_WEBINFO.webUser.username != 'admin'}">
											<a href="#" onclick="viewCms(<s:property value="id"/>)"><s:property value="prcAprislDocmsNo"/></a>
										</c:if>
									</td>
									<td style="text-align: center;" nowrap>
