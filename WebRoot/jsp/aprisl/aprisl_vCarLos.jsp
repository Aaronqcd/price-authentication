...
<table>
	...
	<tr>
		<th>备注信息：</th>
		<td><s:textfield id="remark" name="aprislDto.remark"  cssClass="input3" /></td>
		<th></th>
		<td></td>
	</tr>
	<tr>
		<th>车牌号码：</th>
		<td><s:textfield id="vcPlNm" name="aprislDto.vcPlNm" cssClass="input1" /></td>
		<th>车架号码：</th>
		<td><s:textfield id="vcFrmNm" name="aprislDto.vcFrmNm" cssClass="input1" /></td>
	</tr>
	<tr>
		<th style="width: 150px;">发动机号码：</th>
		<td><s:textfield id="vcEgnNm" name="aprislDto.vcEgnNm" cssClass="input1" /></td>
		<th>车身颜色：</th>
		<td><s:textfield id="vcCarClr" name="aprislDto.vcCarClr" cssClass="input1" /></td>
	</tr>
	<tr>
		<th>燃料种类：</th>
		<td><s:textfield id="vcLosFuelKnd" name="aprislDto.vcLosFuelKnd" cssClass="input1" /></td>
		<th>结构特征：</th>
		<td>
		<s:if test="null==aprislDto.vcLosStruct">
			<s:radio value="1" id="vcLosStruct" name="aprislDto.vcLosStruct" list="#{'1':'电喷','0':'化油器'}"></s:radio>
		</s:if>
		<s:else>
			<s:radio id="vcLosStruct" name="aprislDto.vcLosStruct" list="#{'1':'电喷','0':'化油器'}"></s:radio>
		</s:else>
		</td>
	</tr>
	<tr>
		<th style="width: 150px;">初次登记日期：</th>
		<td>
			<input type="text" name="aprislDto.vcLosInitRegDt" id="vcLosInitRegDt" value="<s:date name="aprislDto.vcLosInitRegDt" format="yyyy-MM-dd"/>"
						readonly="readonly" class="input1"/> 
					<img onclick="WdatePicker({el:'vcLosInitRegDt',readOnly:true});" src="${pageContext.request.contextPath}/images/ril.png"
						width="20px" height="18px" border="0" style="margin-bottom:-5px;margin-left:-25px;" />
		</td>
		<th>购置地点：</th>
		<td><s:textfield id="vcLosBuyAdrs" name="aprislDto.vcLosBuyAdrs" cssClass="input1" /></td>
	</tr>
	<tr>
		<th style="width: 150px;">年检合格日期：</th>
		<td>
			<input type="text" name="aprislDto.vcLosAnnPassDt" id="vcLosAnnPassDt" value="<s:date name="aprislDto.vcLosAnnPassDt" format="yyyy-MM-dd"/>"
						readonly="readonly" class="input1"/> 
					<img onclick="WdatePicker({el:'vcLosAnnPassDt',readOnly:true});" src="${pageContext.request.contextPath}/images/ril.png"
						width="20px" height="18px" border="0" style="margin-bottom:-5px;margin-left:-25px;" />
		</td>
		<th>缴费情况：</th>
		<td><s:textfield id="vcLosPymntStatn" name="aprislDto.vcLosPymntStatn" cssClass="input1" /></td>
	</tr>
	<tr>
		<th style="width: 150px;">标的基准日状况：</th>
		<td><s:textfield id="vcLosArtclRefDtSt" name="aprislDto.vcLosArtclRefDtSt" cssClass="input1" /></td>
		<th style="width: 150px;">市场价格(￥)：</th>
		<td><s:textfield id="orgnlPrc" name="aprislDto.orgnlPrc" cssClass="input1" />元</td>
	</tr>
	<tr>
		<th style="width: 150px;">累计行程(表显)：</th>
		<td><s:textfield id="vcTotlTrip" name="aprislDto.vcTotlTrip" cssClass="input1" />公里</td>
		<th></th>
		<td></td>
	</tr>
	<tr>
		<th>其它信息：</th>
		<td><s:textfield id="othInfo" name="aprislDto.othInfo"  cssClass="input3" /></td>
		<th>使用性质：</th>
		<td>
		<s:if test="null==aprislDto.vcUseNtr">
			<s:radio value="1" id="vcUseNtr" name="aprislDto.vcUseNtr" list="#{'1':'营运','0':'非营运'}"></s:radio>
		</s:if>
		<s:else>
			<s:radio id="vcUseNtr" name="aprislDto.vcUseNtr" list="#{'1':'营运','0':'非营运'}"></s:radio>
		</s:else>
		</td>
	</tr>
	<tr>
		<th>购置时间：</th>
		<td>
		<input type="text" name="aprislDto.buyTm" id="buyTm" value="<s:date name="aprislDto.buyTm" format="yyyy-MM-dd"/>"
						readonly="readonly" class="input1"/> 
					<img onclick="WdatePicker({el:'buyTm',readOnly:true});" src="${pageContext.request.contextPath}/images/ril.png"
						width="20px" height="18px" border="0" style="margin-bottom:-5px;margin-left:-25px;" />
		</td>
		<th style="width: 150px;"><span style="color:red">*</span>价格鉴定基准日期：</th>
		<td>
		<input type="text" name="aprislDto.prcArtclRefDt" id="prcArtclRefDt" value="<s:date name="aprislDto.prcArtclRefDt" format="yyyy-MM-dd"/>"
						readonly="readonly" class="input1"/> 
					<img onclick="WdatePicker({el:'prcArtclRefDt',readOnly:true});" src="${pageContext.request.contextPath}/images/ril.png"
						width="20px" height="18px" border="0" style="margin-bottom:-5px;margin-left:-25px;" />
	...
	<s:if test="aprislDto.TCommission.acptDt != null">
		...
		<td><s:if test="aprislDto.aprislPrc > 0"><s:textfield id="aprislPrc" name="aprislDto.aprislPrc" cssClass="input1"/></s:if>
		...
		<td><s:textfield id="newRate" name="aprislDto.newRate" cssClass="input1" />%</td>
	</tr>
	<s:if test="aprislDto.TCommission.acptDt != null">
	<tr>
		<th colspan="1" style="width: 150px;">价格鉴定备注：</th>
		<td colspan="3"><s:textarea id="prcArtclRmk" name="aprislDto.prcArtclRmk" rows="5" cols="135"></s:textarea></td>
	</tr>
	</s:if>
</table>
