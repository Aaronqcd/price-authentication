	...
	jQuery(document).ready(
			...
			<!-- 珠|宝|玉|工艺 -->
				...
				<table>
					...
					<tr>
						<th>外形：</th>
						<td><s:textfield id="jjcSap" name="aprislDto.jjcSap" cssClass="input1" /></td>
						<th style="width: 150px;">市场价格(￥)：</th>
						<td><s:textfield id="orgnlPrc" name="aprislDto.orgnlPrc" cssClass="input1" />元</td>
					</tr>
					<tr>
						<th>颜色：</th>
						<td><s:textfield id="jjcClr" name="aprislDto.jjcClr" cssClass="input1" /></td>
						<th>重量：</th>
						<td><s:textfield id="jjcWght" name="aprislDto.jjcWght" cssClass="input1" /></td>
					</tr>
					<tr>
						<th>成色：</th>
						<td><s:textfield id="jjcFnns" name="aprislDto.jjcFnns" cssClass="input1" /></td>
						<th>品质等级：</th>
						<td><s:textfield id="jjcGrd" name="aprislDto.jjcGrd" cssClass="input1" /></td>
					</tr>
					<tr>
						<th style="width: 150px;">证书/报告编号：</th>
						<td>
							<s:textfield id="jjcCertRprtNo" name="aprislDto.jjcCertRprtNo" cssClass="input1" />
						</td>
						<th>尺寸：</th>
						<td><s:textfield id="jjcSize" name="aprislDto.jjcSize" cssClass="input1" /></td>
					</tr>
					<tr>
						<th>标的形态：</th>
						<td>
							...
							<s:radio value="1" id="isMjrChng" name="aprislDto.isMjrChng" list="#{'1':'是','0':'否'}"></s:radio>
							</s:if>
							<s:else>
							<s:radio id="isMjrChng" name="aprislDto.isMjrChng" list="#{'1':'是','0':'否'}"></s:radio>
							</s:else>
						</td>
						<th style="width: 150px;">是否有质量、技术等<br>检测、鉴定证书（报告）：</th>
						<td>
						<s:if test="null==aprislDto.jjcIsQltArtclCert">
							<s:radio value="1" id="jjcIsQltArtclCert" name="aprislDto.jjcIsQltArtclCert" list="#{'1':'是','0':'否'}"></s:radio>
						</s:if>
						<s:else>
							<s:radio id="jjcIsQltArtclCert" name="aprislDto.jjcIsQltArtclCert" list="#{'1':'是','0':'否'}"></s:radio>
						</s:else>
						</td>
					</tr>
					<tr>
						...
						<td>
							<input type="text" name="aprislDto.prcArtclRefDt" id="prcArtclRefDt" value="<s:date name="aprislDto.prcArtclRefDt" format="yyyy-MM-dd"/>"
										readonly="readonly" class="input1" size="12"/> 
					...
					<tr>
						...
						<td colspan="3"><s:if test="aprislDto.aprislPrc > 0"><s:textfield id="aprislPrc" name="aprislDto.aprislPrc" cssClass="input1"/></s:if>
					...
					<tr>
						<th style="width: 150px;">价格鉴定备注：</th>
