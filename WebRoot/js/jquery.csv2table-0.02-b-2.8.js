...
(function($) {
	...
	$.csv2table={
		...
		sortNImg : (new Image()).src='../../img/icon-n-orange.gif',         //Dafault sort IMG N
		sortDImg : (new Image()).src='../../img/icon-d-orange.gif',   //Dafault sort IMG D 
		sortAImg : (new Image()).src='../../img/icon-a-orange.gif',   //Dafault sort IMG A
		setting  : [],
		data     : [],
		_rowsAry : [],
		_doc     : document,
	...
	$.fn.csv2table= function (url,setting){ 
		...
		function mkRowsAry(id,data,sortType,colIndex){  
			...
			if(sortType && rewrite){
				...
				if(op.where){ 
					...
					for(var i=_rowsAry.length-1 ;i> 0;i--){ //鏈€绲傝銇痟eader銇伄銇х劇瑕�
		...
		////
		// 涓︺伖鏇裤亪
		// @parame dataAry    涓︺伖鏇裤亪瀵捐薄閰嶅垪
		// @parame sortType   鏄囬爢A|闄嶉爢D
		// @parame colIndex   銈姐兗銉堝垪
		//
		function sortwk(dataAry,sortType,colIndex){
			...
			if(!isNaN(dataAry[0][ci]) || !isNaN(mved3comma)){
				...
				dataAry.sort(function (a,b){ 
						return (b[ci] - a[ci]) ;//闄嶉爢 
				}):
				dataAry.sort(function (a,b){ 
						return (a[ci] - b[ci]);// 鏄囬爢
				})
		...
		function setCSS(id){
			...
			$('#'+id+' table th').css({
				borderColor      : '#777 #999 #777 #bbb',
				borderStyle      : 'solid',
				borderWidth      : '1px 0',
				cursor			 : 'pointer',
				background       : '#f4f4f4 url(img/th-bg.gif)', 
				fontSize         : '12px',
				color            : '#036',
				padding          : '3px 4px',
				textAlign        : 'left'
			...
			$('#'+id+' table td').css({
				borderColor      : '#DEE9F5 #e4e4e4 #DEE9F5 #e4e4e4',
				borderStyle      : 'solid',
				borderWidth      : '1px 0px',
				padding          : '3px 4px',
				fontSize         : '12px'
