    ...
    $.extend({
        FormValidator: {
            ...
            FormValidate: {
				...
				Length: function(target,values){//验证字符串长度是否在这个范围内values为数组，比如【0,60】
					 
					if( (GetLength(target[0]) >= values[0]) && (GetLength(target[0]) <= values[1]) ){
						return true;
					}else{
						return false;
					}
				...
				IsPostCode: function(target){//验证邮编
					 var pattern=/^[0-9]{6}$/;
					if(pattern.exec(target.val())){
						return true;
					}else{
						return false;
					}
				...
				IsFax: function(target){//验证传真和普通电话
				...
				IsMobile: function(target){//验证手机号码
		            ...
		            var reg3 = /^0\d{10,11}$/;
					var myMobile = false;
					if (reg0.test(target.val()))myMobile=true;
		            if (reg1.test(target.val()))myMobile=true;
		            if (reg2.test(target.val()))myMobile=true;
		            if (reg3.test(target.val()))myMobile=true;
					
					return myMobile;
					
				},
				
				IsEmail: function(target){//验证email
